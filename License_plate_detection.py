...
# Correcting the number plate format.(num-->alpha and alpha -->num)
def correct_plate_format(ocr_text):
    ...
    if len(ocr_text) != 7:
        return ""   #Discard the wrong length plate numbers.
    
    corrected = []
    for i, ch in enumerate(ocr_text):
        if i < 2 or i >= 4 :  #Alphabets
            if ch.isdigit() and ch in mapping_num_to_alpha:
                corrected.append(mapping_num_to_alpha[ch])
            elif ch.isalpha():
                corrected.append(ch)
            else:
                return "" #Discard the wrong character.
...
# Function to detect and recognize license plates in a video stream.
...
#Number plate stabilisation buffer.
...
def get_stable_plate(box_id, new_plate):
    if new_plate:
        plate_history[box_id].append(new_plate)
...
# Video for inferring.
input_video = 'License_plate_input.mp4'
...
while cap.isOpened():
    ...
    for r in results:
        ...
        for box in boxes:
            ...
            # OCR with correection.
            text = recognize_plate(plate_crop)

            # Stabilise text using history.
            ...
            if plate_crop.size > 0:
                ...
                if oy2 <= frame.shape[0] and ox2 <= frame.shape[1]:
                    ...
                    if stable_text:
                        cv2.putText(frame, stable_text, (ox1, oy1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 0, 0), 6)
                        cv2.putText(frame, stable_text, (ox1, oy1 - 20), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (255, 255, 255), 3)
